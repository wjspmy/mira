...
**M0 + M1 已完成，M2 文件管理核心已完成、进入收尾回归**：已具备日常写作与基础工作区体验。

M2 当前已覆盖：文件树、多标签、最近打开、新建/重命名/移动/删除、文件监听、外部移动同步、图片本地化。后续收尾重点见 [docs/known-issues.md](docs/known-issues.md)：复杂文件系统事件回归、路径沙箱、编码/BOM/行尾策略。

路线图：~~M0 脚手架~~ ✅ → ~~M1 核心~~ ✅ → M2 文件管理（收尾回归）→ M3 生产力 → M4 打磨发布 → M5 导出。
