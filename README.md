...
**M0 + M1 + M2 已完成，M3 已完成，M4 进行中**：已具备日常写作、工作区与生产力体验（快捷键、菜单、命令面板、源码模式、自定义 CSS、大文件提示）。

M2 已覆盖并验收：文件树、多标签、最近打开、新建/重命名/移动/删除、文件监听、外部移动同步、图片本地化、路径沙箱、编码/BOM/行尾保留，以及 dirty / 未命名文档草稿恢复。

M3 已覆盖并验收：快捷键系统与自定义设置、Typora 风格菜单栏与底部状态栏、命令面板与快速打开、源码模式（CodeMirror 6，切换保留光标/选区）、自定义 CSS（作用域注入 + 热加载）、大文件自动源码模式。

后续进入 M4 打磨发布（性能、三端打包、自动更新、品牌视觉）。
