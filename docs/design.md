...
| **M3 生产力增强** | 进阶体验 | 快捷键系统与自定义设置、Typora 风格菜单栏、底部状态栏路径展示、命令面板、源码模式开关（CodeMirror 6）、自定义 CSS |
...
当前文件完整路径从顶部移到底部状态栏。底部状态栏分三段：左侧显示操作状态消息，中间显示当前文件路径（长路径省略，hover 显示完整路径），右侧显示保存状态（`已保存` / `● 未保存`）。顶部 tab 只保留文件名，减少长 Windows 路径对主操作区的干扰。
