...
| **M3 生产力增强** ✅ | 进阶体验 | 已完成并验收：快捷键系统与自定义设置、Typora 风格菜单栏与底部状态栏、命令面板与快速打开、源码模式（CodeMirror 6，切换保留光标/选区）、自定义 CSS（作用域注入 + 文件热加载）、大文件自动降级源码模式（虚拟滚动） |
| **M4 打磨发布** | 发布前优化 | 核心已完成：设置（主题/字号/自动保存/图片策略）、大文件源码降级、CSS 热加载；剩余：三端打包、自动更新签名、品牌视觉、完整 E2E |
| **M5 导出（后续）** | 增量能力 | 已交付 HTML / PDF（打印）/ Word (.doc) / PNG |
...
| 操作 | 默认快捷键 | 说明 |
| --- | --- | --- |
| 新建 / 打开 / 保存 | Mod+N / Mod+O / Mod+S | Mod 在 Windows/Linux 为 Ctrl，在 macOS 为 Cmd |
| 打开文件夹 | Mod+Shift+O | 打开工作区根目录 |
| 关闭当前标签页 | Mod+W | 若有未保存内容会沿用关闭确认 |
| 切换标签页 | Mod+Tab / Mod+Shift+Tab / Mod+1~9 | 支持前后切换和按序号切换 |
| 加粗 / 斜体 / 行内代码 / 插入链接 | Mod+B / Mod+I / Mod+E / Mod+K | 编辑器聚焦时仍可用 |
| 切换主题 | Mod+Shift+L | 浅色 / 深色切换 |
| 快捷键设置 | Mod+, | 打开自定义快捷键设置弹窗 |
| 命令面板 | Mod+Shift+P | 搜索并执行命令，或快速打开文件 |

快捷键由统一 registry 管理，用户自定义配置保存到本机 `localStorage`；设置界面负责冲突检测、恢复单项默认和恢复全部默认。命令面板、快速打开、源码模式后续复用同一套命令/快捷键基础设施。

### 14.7 应用菜单栏与状态栏布局（Task 11）

顶部 chrome 采用 Typora 类似的菜单栏，而不是持续堆叠工具按钮。第一版使用 Vue 自绘 Web 菜单，暂不接入 Tauri 原生菜单，以便直接复用前端命令注册表、快捷键配置和 Pinia 状态。

菜单分组：

| 菜单 | 内容 |
| --- | --- |
| 文件 | 新建、打开文件、打开文件夹、保存、关闭标签、最近打开 |
| 编辑 | 撤销、重做、后续查找/替换 |
| 格式 | 加粗、斜体、行内代码、插入链接、无序列表、有序列表、引用、代码块 |
| 视图 | 切换主题、后续侧边栏/源码模式开关 |
| 工具 | 命令面板、快捷键设置 |
| 帮助 | 关于 Mira、版本信息 |

“最近打开”属于文件入口，应收进 `文件` 菜单；左侧栏聚焦工作区文件树，避免最近列表与文件树竞争空间。菜单项右侧显示当前快捷键，快捷键文本来自统一 shortcut store，用户修改快捷键后菜单显示同步更新。

当前文件完整路径从顶部移到底部状态栏左侧（长路径省略，hover 显示完整路径），右侧显示保存状态（`已保存` / `● 未保存`）。顶部 tab 只保留文件名，减少长 Windows 路径对主操作区的干扰。

Task 11 实现文件：`src/components/AppMenuBar.vue` 负责菜单交互，`src/menus/appMenu.ts` 负责菜单模型与最近打开映射，`src/App.vue` 复用现有命令执行与快捷键系统。

菜单中的撤销/重做复用当前文档自己的编辑器命令；每个 tab 拥有独立的 Tiptap `Editor` 实例，让 A→B→A 后仍可撤销 A 的编辑，同时避免内容、选区和撤销历史串到其他文件。

Task 11 已完成并验收：菜单栏、文件菜单最近打开、底部路径状态栏、帮助提示和 per-tab Editor 撤销隔离均已通过用户手工测试。

---

### 14.8 命令面板与快速打开（Task 12）

命令面板复用统一的快捷键 registry，默认通过 `Mod+Shift+P` 打开（Windows/Linux 显示为 `Ctrl+Shift+P`）。面板优先匹配并展示应用命令，也会聚合已打开标签、最近打开文件和当前工作区的 Markdown / 文本文件；输入文件名或路径片段即可打开文件。

工作区文件由 Tauri 后端按需递归索引，最多返回 2,000 项；索引只允许当前已授权的工作区根目录，跳过 `.git`、`node_modules`、`target`、`dist`、`.vite`、隐藏目录及符号链接，只收集 `.md`、`.markdown`、`.mdx` 与 `.txt`。面板打开后不立即扫描工作区，先保证命令搜索与输入响应；当用户输入至少 2 个字符并短暂停顿后，再延迟索引工作区文件，索引期间只显示轻提示“正在准备工作区文件…”。面板支持方向键选择、Enter 执行、Esc 关闭，并在无工作区、索引失败和无匹配结果时提供明确反馈。
...
| 层 | 范围 | 工具 |
| --- | --- | --- |
| 单元 | 序列化器、路径归一化 / 移动前缀替换、workspace / recent / session store 回归、编码探测、路径沙箱、草稿恢复判定 | Vitest |
