...
- 顶部按钮已改为 Typora 风格 Web 菜单栏。
- “最近打开”已移动到“文件”菜单。
- 左侧栏已聚焦工作区文件树，不再显示最近打开列表和“右键操作”提示。
- 当前文件路径已移动到底部状态栏左侧，保存状态显示在右侧。
- 菜单项显示当前快捷键，并跟随自定义快捷键设置更新。
- “帮助 > 关于 Mira” 已接入信息弹窗。
- 撤销/重做改为基于当前文档自己的 Tiptap Editor 实例执行；每个 tab 拥有独立 Editor，避免 A/B 文档内容、选区和撤销历史串联。

**User verification checklist:**

- 文件菜单的新建/打开/打开文件夹/保存/关闭标签正常。
- 文件菜单最近打开正常。
- 左侧栏只显示工作区文件树。
- 底部状态栏左侧显示路径、右侧显示保存状态。
- A→B→A 后，A 可撤销自己的编辑。
- A 切到 B 后点击 B 编辑区，B 不会变成 A。
- 帮助菜单有提示信息。
